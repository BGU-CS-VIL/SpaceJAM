...
from dino_vit_features.vit_extractor_v1 import ViTExtractor
import numpy as np
import faiss
...
def find_cosegmentation(image_paths: List[Union[str, Path]], elbow: float = 0.975, load_size: int = 224, layer: int = 11,
    ...
    for image_path in image_paths:
        ...
        # removing outlier images requires the cls descriptor.
        include_cls = remove_outliers
        descs = extractor.extract_descriptors(image_batch.to(device), layer, facet, bin, include_cls).detach().cpu().numpy()
        ...
        if remove_outliers:
            cls_descriptor, descs = torch.from_numpy(
                descs[:, :, 0, :]), descs[:, :, 1:, :]
        ...
        if low_res_saliency_maps:
            if load_size is not None:
                low_res_load_size = (
                    curr_load_size[0] // 2, curr_load_size[1] // 2)
            else:
                low_res_load_size = curr_load_size
            image_batch, _ = saliency_extractor.preprocess(
                image_path, low_res_load_size)

        saliency_map = saliency_extractor.extract_saliency_maps(image_batch.to(device)).detach().cpu().numpy()
        curr_sal_num_patches, curr_sal_load_size = saliency_extractor.num_patches, saliency_extractor.load_size
        if low_res_saliency_maps:
            reshape_op = transforms.Resize(
                curr_num_patches, transforms.InterpolationMode.NEAREST)
            saliency_map = np.array(reshape_op(Image.fromarray(
                saliency_map.reshape(curr_sal_num_patches)))).flatten()
        else:
        ...
        if save_dir is not None:
            ...
            ax.axis('off')
            ax.imshow(
                saliency_maps_list[-1].reshape(num_patches_list[-1]), vmin=0, vmax=1, cmap='jet')
            fig.savefig(save_dir_saliencies /
                        f'{Path(image_path).stem}_saliency_map.png', bbox_inches='tight', pad_inches=0)
            plt.close(fig)
            if args.save_orig_imgs:
                image_pil.save(save_dir_images /
                               f'{Path(image_path).stem}_resized.png')
    ...
    if remove_outliers:
        ...
        similarities_to_mean = cos_sim(
            all_cls_descriptors, mean_cls_descriptor)
        ...
        for idx, (image_path, descriptor, saliency_map, pil_image, num_patches, load_size) in enumerate(zip(image_paths,
            ...
            (inlier_image_paths if idx in inliers_idx else outlier_image_paths).append(
                image_path)
            (inlier_descriptors if idx in inliers_idx else outlier_descriptors).append(
                descriptor)
            (inlier_saliency_maps if idx in inliers_idx else outlier_saliency_maps).append(
                saliency_map)
            (inlier_image_pil if idx in inliers_idx else outlier_image_pil).append(
                pil_image)
            (inlier_num_patches if idx in inliers_idx else outlier_num_patches).append(
                num_patches)
            (inlier_load_size if idx in inliers_idx else outlier_load_size).append(
                load_size)
    ...
    all_descriptors = np.ascontiguousarray(
        np.concatenate(descriptors_list, axis=2)[0, 0])
    ...
    sampled_descriptors_list = [
        x[:, :, ::sample_interval, :] for x in descriptors_list]
    all_sampled_descriptors = np.ascontiguousarray(
        np.concatenate(sampled_descriptors_list, axis=2)[0, 0])
    normalized_all_sampled_descriptors = all_sampled_descriptors.astype(
        np.float32)
    # in-place operation
    ...
    for n_clusters in n_cluster_range:
        ...
        algorithm = faiss.Kmeans(
            d=normalized_all_sampled_descriptors.shape[1], k=n_clusters, niter=30, nredo=3)
        algorithm.train(normalized_all_sampled_descriptors.astype(np.float32))
        squared_distances, labels = algorithm.index.search(
            normalized_all_descriptors.astype(np.float32), 1)
        objective = squared_distances.sum()
        sum_of_squared_dists.append(
            objective / normalized_all_descriptors.shape[0])
        if (len(sum_of_squared_dists) > 1 and sum_of_squared_dists[-1] > elbow * sum_of_squared_dists[-2]):
            break

    num_labels = np.max(n_clusters) + 1
    num_descriptors_per_image = [num_patches[0]*num_patches[1]
    ...
    if save_dir is not None:
        ...
        for image_path, num_patches, label_per_image in zip(image_paths, num_patches_list, labels_per_image):
            fig, ax = plt.subplots()
            ax.axis('off')
            ax.imshow(label_per_image.reshape(num_patches),
                      vmin=0, vmax=num_labels-1, cmap=cmap)
            fig.savefig(save_dir_clustering /
                        f'{Path(image_path).stem}_clustering.png', bbox_inches='tight', pad_inches=0)
            plt.close(fig)
    ...
    for image_labels, saliency_map in zip(labels_per_image, saliency_maps_list):
        for label in range(num_labels):
            ...
            if label_saliency > thresh:
                votes[label] += 1
    salient_labels = np.where(votes >= np.ceil(
        num_images * votes_percentage / 100))
    ...
    for img, labels, num_patches, load_size in zip(image_pil_list, labels_per_image, num_patches_list, load_size_list):
        ...
        resized_mask = np.array(Image.fromarray(mask).resize(
            (load_size[1], load_size[0]), resample=Image.LANCZOS))
        try:
            ...
            forground_mask = np.array(Image.fromarray(
                forground_mask).resize(img.size, Image.NEAREST))
            background_mask = cv2.erode(np.uint8(1 - resized_mask), kernel)
            background_mask = np.array(Image.fromarray(
                background_mask).resize(img.size, Image.NEAREST))
            full_mask = np.ones(
                (load_size[0], load_size[1]), np.uint8) * cv2.GC_PR_FGD
            full_mask[background_mask == 1] = cv2.GC_BGD
            full_mask[forground_mask == 1] = cv2.GC_FGD
            bgdModel = np.zeros((1, 65), np.float64)
            fgdModel = np.zeros((1, 65), np.float64)
            cv2.grabCut(np.array(img), full_mask, None, bgdModel,
                        fgdModel, 5, cv2.GC_INIT_WITH_MASK)
            grabcut_mask = np.where((full_mask == 2) | (
                full_mask == 0), 0, 1).astype('uint8')
        except Exception:
        ...
        grabcut_mask = np.array(grabcut_mask, dtype=bool)
        segmentation_masks.append(grabcut_mask)

    if remove_outliers:
        outlier_segmentation_masks = []
        for load_size in outlier_load_size:
            outlier_segmentation_masks.append(
                Image.fromarray(np.zeros(load_size, dtype=bool)))
        ...
        for idx in range(len(image_paths)):
            if idx in inliers_idx:
                ...
                final_segmentation_masks.append(
                    outlier_segmentation_masks.pop(0))
...
def draw_cosegmentation(segmentation_masks: List[Image.Image], pil_images: List[Image.Image]) -> List[plt.Figure]:
    ...
    for seg_mask, pil_image in zip(segmentation_masks, pil_images):
        ...
        masked_image_transparent = np.concatenate(
            (masked_image, 255. * np_mask.astype(np.int32)[..., None]), axis=-1)

        # create chessboard bg
        chessboard_bg = np.zeros(np_image.shape[:2])
        chessboard_edge = 10
        chessboard_bg[[x // chessboard_edge %
                       2 == 0 for x in range(chessboard_bg.shape[0])], :] = 1
        chessboard_bg[:, [x // chessboard_edge % 2 == 1 for x in range(chessboard_bg.shape[1])]] = \
            1 - chessboard_bg[:, [x // chessboard_edge %
                                  2 == 1 for x in range(chessboard_bg.shape[1])]]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Facilitate ViT Descriptor cosegmentations.')
    parser.add_argument('--folder_path', type=str,
                        required=True, help='The root dir of image sets.')
    ...
    parser.add_argument('--bin', default='False', type=str2bool,
                        help="create a binned descriptor if True.")
    parser.add_argument('--thresh', default=0.065, type=float,
                        help='saliency maps threshold to distinguish fg / bg.')
    parser.add_argument('--elbow', default=0.975, type=float,
                        help='Elbow coefficient for setting number of clusters.')
    ...
    parser.add_argument('--remove_outliers', default='False',
                        type=str2bool, help="Remove outliers using cls token.")
    parser.add_argument('--outliers_thresh', default=0.7,
                        type=float, help="Threshold for removing outliers.")
    ...
    parser.add_argument('--save_orig_imgs', default=False,
                        type=bool, help="if true, will save original images.")
    ...
    with torch.no_grad():
        ...
        curr_images = [x for x in images_path.iterdir() if x.suffix.lower() in [
            '.jpg', '.png', '.jpeg']]
        ...
        for image, binary_mask, chessboard_fig in zip(curr_images, seg_masks, chessboard_bg_figs):
            binary_mask.save(save_dir / "mask" /
                             f'{Path(image).stem}_mask.png')
            chessboard_fig.savefig(
                save_dir / "vis" / f'{Path(image).stem}_vis.png', bbox_inches='tight', pad_inches=0)
        plt.close('all')
